...
# Test Generation: jester

## Context
**Description:** a full self learning ai and problem solver and anomaly detection and science tester ultimate tool
...
Feature: Jester AI Core
  ...
  # ==========================================
  Scenario: [API] Submit data for anomaly detection
    Given the jester ai is active
    When the client sends POST request to "/api/anomaly-detect" with valid dataset
    Then the response status should be 200
    And the response body should contain "anomalies_found"

  Scenario: [API] Submit data with invalid format
    Given the jester ai is active
    When the client sends POST request to "/api/anomaly-detect" with invalid dataset
    Then the response status should be 400
    And the response error should be "Invalid dataset format"

  # ==========================================
  # UI SCENARIOS (User Actions & UI)
  # ==========================================
  Scenario: [UI] Run self learning training
    Given the user is on the jester dashboard
    When the user clicks the "Start Self Learning" button
    Then the user should see "Training initiated" message
...
## Your Task
Implement `.tdad/workflows/default/jester/jester.action.js` and `.tdad/workflows/default/jester/jester.test.js`.

...
async function main() {
  ...
  await shot('c_A'); await page.waitForTimeout(4000); await shot('c_B');
  console.log('focused-wait-4s   :', JSON.stringify(diff(`${tmp}/c_A.png`, `${tmp}/c_B.png`).moving));
  // hold RIGHT 3s focused
  await page.evaluate(() => { window.__GM.loadState(window.__SAVED); });
  await page.waitForTimeout(1000);
  await shot('r_A'); await press(7, 3000); await shot('r_B');
  console.log('focused-right-3s  :', JSON.stringify(diff(`${tmp}/r_A.png`, `${tmp}/r_B.png`)));
  await browser.close();
}
